...
- `DEFAULT_CHANNEL_ID` — id канала для публикации.
- `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD` — настройки PostgreSQL.
- `REDIS_HOST`, `REDIS_PORT`, `REDIS_DB` — настройки Redis для FSM.
- `APP_ENV` — среда запуска, по умолчанию `local`.
...
- открытие черновика из `/drafts` обратно в preview;
- публикация черновика из `/drafts`;
- удаление черновика из активного списка;
...
4. Выполните `/drafts` и проверьте для одного черновика:
   - `Открыть`
   - `Опубликовать`
   - `Удалить`
...
- `Черновик не найден или уже недоступен.`
  Причина: черновик уже опубликован, удалён или не принадлежит текущему пользователю.
